 ...
 * @param {String} opts.ssdpInterface Only receice specific interface broadcast
 ...
 * @param {String} opts.sendSock Sock to send packets (useful for send packets bind on specfic interface)
 *
...
function SSDP(opts, sock) {
  ...
  opts = opts || {}
  this._ssdpInterface = opts.ssdpInterface || false;
  ...
  this.sendSock = this.sock
  if (opts.sendSock) {
    this.sendSock = opts.sendSock
    this.sendSock.unref()
  }
...
SSDP.prototype._start = function (port, host, cb) {
  ...
  this.sock.on('listening', function onSocketListening() {
    ...
    function addMembership() {
      try {
        if (self._ssdpInterface === false) {
          self.sock.addMembership(self._ssdpIp)
        } else {
          self.sock.addMembership(self._ssdpIp, self._ssdpInterface)
        }
...
SSDP.prototype._parseMessage = function (msg, rinfo) {
  ...
  if (httpHeader.test(type)) {
    ...
    // add a random delay in 5 seconds to prevent large amount of ssdp response
    var self = this;
    setTimeout(function () {
      self._parseCommand(msg, rinfo);
    }, Math.random() * 5000);
  }
}
...
SSDP.prototype._send = function (message, host, port, cb) {
  ...
  self.sendSock.send(message, 0, message.length, port, host, cb)
}
